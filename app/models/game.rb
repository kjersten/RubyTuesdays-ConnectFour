class Game < ActiveRecord::Base
  ...
  attr_accessible :name, :red_player_name, :blue_player_name, :board, :created_at, :status, :current_player
  ...
  validates_presence_of [:name, :red_player_name, :blue_player_name], :on => :create
  ...
  def board_position(coords)
    raise ArgumentError unless coords_valid?(coords)
  ...
  def coords_valid?(coords)
    x = coords[0]
    y = coords[1]
    0 <= x && x <= NUM_COLUMNS - 1 && 0 <= y && y <= NUM_ROWS - 1
  end
